...
<template>
  <div class="group bg-secondary-dark pb-10 rounded-[40px] overflow-hidden shadow-lg">
    ...
    <div class="flex flex-col xl:flex-row xl:items-center justify-between px-6 gap-5 md:gap-10" :class="{'mt-10': isFirstProject}">
      ...
      <BaseButton variant="outline" class="group/button min-w-max w-full xl:w-auto">
        View Case
        <SvgIcon name="arrow-right" class="transition group-hover/button:-rotate-45 ml-2 w-4 h-4" />
      </BaseButton>
    </div>
  </div>
</template>
